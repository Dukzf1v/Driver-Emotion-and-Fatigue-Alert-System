...
def run(args):
    ...
    while True:
        ...
        if landmark_data is not None:
            ...
            p_sad   = float(probs[config.SADNESS_CLASS_IDX]) if len(probs) > config.SADNESS_CLASS_IDX else 0.0
            ...
            # - Sadness (crying/looking down) may squint eyes (reduces EAR)
            t_ear_factor = 1.0 - (0.20 * p_happy) - (0.15 * p_sad)
