// Quản lý xe và biểu đồ
let vehicles = {}; // vehicleId -> vehicleData
let selectedVehicleId = null;
let chartInstance = null;
let currentTab = 'tabLive';

// Lưu trữ lịch sử dữ liệu để vẽ biểu đồ cho từng xe (tối đa 60 điểm = 1 phút)
...
function initChart() {
    ...
    // Cấu hình giao diện biểu đồ tối của Chart.js tương thích với style.css
...
// 2. Chuyển đổi xe hiển thị
function selectVehicle(vehicleId) {
    selectedVehicleId = vehicleId;
    
    // Cập nhật class active trong danh sách sidebar
    ...
    // Hiện vùng chi tiết, ẩn placeholder
    noVehiclesPlaceholder.style.display = 'none';
    vehicleDetails.style.display = 'block';
    
    // Cập nhật lại biểu đồ với lịch sử của xe này
    ...
    chartInstance.update('none'); // Cập nhật không chạy animation để tăng hiệu năng
    
    // Cập nhật giao diện thông số xe
...
// 3. Render lại danh sách xe trên Sidebar
...
// 4. Tính toán và cập nhật mũi tên xu hướng (Trend)
...
// 5. Cập nhật chi tiết xe lên giao diện chính
...
// 6. Xử lý chuyển đổi TAB giao diện
function switchTab(tabId) {
    currentTab = tabId;
    
    // Đổi active link của Nav
    document.querySelectorAll('.nav-links a').forEach(a => a.classList.remove('active'));
    document.getElementById(tabId).classList.add('active');
    
    // Các phần tử view chính
...
// 7. Gọi API nạp dữ liệu Lịch Sử Cảnh Báo
...
// 8. Gọi API nạp dữ liệu thống kê Tài Xế
function fetchDrivers() {
    fetch('/api/drivers')
        ...
        .then(data => {
            ...
            data.forEach(d => {
                // Xác định class cho điểm số an toàn
...
// 9. Xử lý kết nối SSE tới Flask Server
function initSSE() {
    ...
    source.onmessage = function(event) {
        ...
        // A. Khởi tạo danh sách từ Server
        if (msg.type === 'init') {
            vehicles = msg.vehicles;
            
            // Khởi tạo bộ lưu trữ lịch sử cho các xe
        ...
        // B. Cập nhật dữ liệu từ xe (Real-time update)
            ...
            // Nếu chưa chọn xe nào, hoặc đây là xe duy nhất mới kết nối -> tự động chọn xe
            if (!selectedVehicleId || isFirstTime) {
                selectVehicle(vid);
            }
            
            // Nếu xe này đang được chọn -> cập nhật giao diện chi tiết + đẩy điểm mới vào chart
            if (selectedVehicleId === vid) {
                updateVehicleDetailUI(msg.data);
                
                // Cập nhật Chart.js trực tiếp (mượt và nhanh hơn)
            ...
            // Nếu có cảnh báo và đang ở các tab phụ -> Tự động cập nhật không cần reload thủ công
        ...
        // C. Cập nhật trạng thái kết nối (online/offline) từ Server
        else if (msg.type === 'status_change') {
            Object.keys(msg.updates).forEach(vid => {
                if (vehicles[vid]) {
                    vehicles[vid].status = msg.updates[vid];
                    
                    // Nếu xe bị offline -> xóa dữ liệu cũ, chỉ giữ cấu trúc cơ bản
            ...
            // Nếu xe đang chọn bị thay đổi trạng thái -> cập nhật lại chi tiết
            if (selectedVehicleId && msg.updates[selectedVehicleId]) {
                updateVehicleDetailUI(vehicles[selectedVehicleId]);
            }
            
            // Đồng bộ tab phụ khi có trạng thái thay đổi
            if (currentTab === 'tabHistory') fetchHistory();
            if (currentTab === 'tabDrivers') fetchDrivers();
        }
    };
...
// 10. Khởi chạy khi tài liệu load xong
document.addEventListener('DOMContentLoaded', () => {
    initChart();
    initSSE();
    
    // Gắn sự kiện điều hướng Tabs
    ...
    // Gắn sự kiện tìm kiếm & lọc trên tab Lịch sử
